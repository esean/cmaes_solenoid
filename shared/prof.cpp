...
{
    // handle update times
    away_time_recent = update.getElapsedTimeInMicroSec();
    update.reset();
...
//--------------------------
double prof::get_current_timer_us()
{
    return tmr.getElapsedTimeInMicroSec();
}
...
//  y = x0*m + x1*(1-m)
//
double prof::flt_avg(const double newv, const double oldv, const double coeff)
{
